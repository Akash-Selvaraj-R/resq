    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="bg-surface/50 backdrop-blur-sm border-b border-zinc-800/20">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row md:items-center md:justify-between py-4">
          <div className="flex items-center space-x-3">
            <ArrowLeft className="h-5 w-5 text-zinc-400 hover:text-zinc-300" onClick={() => router.push(`/sos/confirm?type=${type}`)} />
      ...
      <div className="flex-1 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col">
          ...
          >
            <div className={`bg-${info.color}/10 rounded-xl p-6 mb-8 border border-${info.color}/20`}>
              <div className="flex items-center space-x-4">
                <info.icon className={`h-6 w-6 text-${info.color}`} />
                <div>
                  <h2 className={`font-semibold text-${info.color}`}>{info.title}</h2>
                  <p className="text-zinc-300 text-sm">{info.description}</p>
                </div>
              </div>
            </div>
          </Motion>
              ...
              {isLoading ? (
                ...
                <>
                  {responders.map((responder) => (
                    ...
                    >
                      <div className={`p-4 rounded-lg border border-zinc-800/30 bg-surface/50 hover:bg-surface/70 transition-colors`}>
                        <div className="flex items-center space-x-3">
                          <div className="flex-shrink-0 h-8 w-8 flex items-center justify-center bg-primary/20 rounded-lg">
                          ...
                          >
                            {responder.status === "accepting" ? "Accept Help" : "ETA 2min"}
                          </Button>
                        </div>
                      </div>
                    </Motion>
                  ...
                  {isLoading ? (
                    ...
                    <>
                      {hospitals.map((hospital) => (
                        <div key={hospital.name} className="flex items-center justify-between p-3 rounded-lg border border-zinc-800/20 bg-surface/50 hover:bg-surface/70 transition-colors">
                          <div className="flex items-center space-x-3">
                            <MapPulse className="h-4 w-4 text-primary" />
                            <div>
              ...
              >
                <div className="space-y-4">
                  ...
                  {isLoading ? (
                    ...
                    <>
                      {police.map((station) => (
                        <div key={station.name} className="flex items-center justify-between p-3 rounded-lg border border-zinc-800/20 bg-surface/50 hover:bg-surface/70 transition-colors">
                          <div className="flex items-center space-x-3">
